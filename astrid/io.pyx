...
def start_voice(event_loop, executor, instrument, params, buf_q, play_q, event_q):
    ...
    if hasattr(instrument.renderer, 'before_async'):
        # async before callback may write to 
        # ctx bus or just do something async
        event_loop.run_in_executor(executor, instrument.renderer.before_async, ctx)
    ...
    if hasattr(instrument.renderer, 'play'):
        onsets = getattr(instrument.renderer, 'onsets', None)
    ...
    if hasattr(instrument.renderer, 'done'):
        ...
        event_loop.run_in_executor(executor, instrument.renderer.done, ctx)
